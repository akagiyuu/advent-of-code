...
fn solve(points: &[Point]) -> u64 {
    ...
    for &(_, u, v) in dist.iter().take(EDGE_COUNT) {
        dsu.unite(u, v);
    }

    let mut parents = HashSet::new();
    for u in 1..=n {
        parents.insert(dsu.find(u));
    }
    let mut sz = Vec::with_capacity(parents.len());
    for parent in parents {
        sz.push(dsu.sz[parent]);
    }
    sz.sort();

    let mut res = 1;

    for x in sz.iter().rev().take(SZ_COUNT) {
        res *= x;
    }
